...
(function () {
  ...
  function isAddressValid(value) {
    ...
    var localPart = email.slice(0, firstAt);
    var rawDomain = email.slice(firstAt + 1);
    ...
    return validTopLevelDomains.has(labels[labels.length - 1]);
  }

  function updateValidity(emailInput) {
    ...
    var isValid = isAddressValid(value);
    emailInput.setCustomValidity(isValid ? "" : errorMessage);
    return isValid;
  }

  function stopInvalidSubmission(event, emailInput) {
    emailInput.value = emailInput.value.trim();

    if (updateValidity(emailInput)) {
      return;
    }
  ...
  function initialize() {
    document.querySelectorAll("form").forEach(function (form) {
      ...
      emailInput.addEventListener("blur", function () {
        updateValidity(emailInput);
      });

      form.addEventListener(
        "click",
        function (event) {
          var submitControl = event.target.closest(
            'button[type="submit"], input[type="submit"], button:not([type])'
          );

          if (submitControl && submitControl.form === form) {
            stopInvalidSubmission(event, emailInput);
          }
        },
        true
      );
